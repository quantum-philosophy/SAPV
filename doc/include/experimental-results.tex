...
The number of measured dies $\nrdies$ is 20 (see \fref{wafer-measured}). These dies are chosen as follows: the first one is placed in the middle of the wafer, and the other are selected sequentially such that the total distance from the already picked dies and the new one to the rest of the dies is minimized.
...
Without loss of generality, the leakage model used in \eref{total-power} is constructed based on SPICE simulations of reference electrical circuits using the Predictive Technology Model \cite{ptm} followed by a curve fitting procedure.
...
Let us turn to the statistical model in \sref{statistical-model}.
In the covariance function given by \eref{covariance-function}, the weight parameter $\eta$ and the two length-scale parameters $\ell_\SE$ and $\ell_\OU$ should be set according to the correlation patterns typical for the production process at hand; we set $\eta$ to 0.7 and $\ell_\SE$ and $\ell_\OU$ to half the radius of the wafer.
The threshold parameter in the model order reduction procedure described in \aref{kl-expansion} should be high enough to preserve a sufficiently large portion of the variance of the data; we set it to $0.99$ to preserve $99\%$ of this variance. The resulting dimensionality $\nvars$ of $\vz$ in \eref{kl-approximation} was found to be 27--28.
The parameters $\mu_0$ and $\tau_\u$ of the priors in \eref{mu-u-prior} and \eref{sigma2-u-prior} are process-specific; we set $\mu_0$ to $45~\text{nm}$ and $\tau_\u$ to $5\%$ of $\mu_0$.
...
In \eref{sigma2-noise-prior}, $\tau_\noise$ represents the precision of the equipment utilized for temperature measurements, which can be found in the specification of this equipment; we set $\tau_\noise$ to $1~\text{K}$. The parameter $\nu_\noise$ has the same interpretation as $\nu_\u$, and we also set it to ten.
In \eref{proposal}, $\nu$ and $\alpha$ are tuning parameters, which should be set based on experiments; we set $\nu$ to eight and $\alpha$ to 0.5.
...
For parallelization, we utilize four processors.\footnote{All the experiments have been conducted on a GNU/Linux machine with Intel Core i7 2.66~GHz and 8~GB of RAM.}
...
Now we inspect the quality of the proposal distribution (recall \sref{bayesian-inference} and \sref{optimization}). To this end, we plot (not shown due to the lack of space) the posterior given in \eref{log-posterior} for each drawn sample and observe that the obtained curve rapidly fluctuates around a constant level indicating that the constructed Markov chain vividly explores the underlying probability space.
Another test commonly used in statistics to assess the quality of the proposal distribution is to depict this distribution, given in \eref{log-posterior}, side-by-side with the tailored proposal distribution, given in \eref{proposal}, for each parameter conditioned on the other parameters being at their posterior modes.
...
In the following four experiments, we modify only one parameter of the default configuration at a time: the number of processing elements on a die $\nprocs$, the number of measured dies on the wafer $\nrprocs$, the number of temporal measurements $\nrsteps$, and the deviation of the noise $\sigma_\noise$.
...
In this and the following tables, we report the optimization (Stage~2 in \fref{algorithm}) and sampling (Stage~3 in \fref{algorithm}) times separately (given in minutes); moreover, the sampling time is given for two cases: sequential and parallel computing, which is followed by the total time and error. The computational time of the post-processing phase (Stage~4 in \fref{algorithm}) is not given as it is negligibly small.
...
In this subsection, we change the number of dies $\nrdies$ for which the measurement data are available in the input data set $\Data$ (correspondingly, $\ndies - \nrdies$ dies are left unobserved in \fref{wafer-measured}). The considered scenarios are 1, 10, 20, 40, 80, and 160 measured dies. The results are reported in \tref{spatial-measurements}.
...
In this subsection, we sweep the number of moments of time $\nrsteps$ for which the measurement data are available in the input data set $\Data$ (correspondingly, $\nsteps - \nrsteps$ time moments, see \fref{wafer-measured}, are discarded while evaluating $\model$). The scenarios are 1, 10, 20, 40, 80, and 160 time moments. The results are aggregated in \tref{temporal-measurements}.
