...
Since drawing samples directly from the posterior distribution in \eref{posterior} is not possible (see \sref{bayesian-inference}), we utilize the Metropolis-Hastings algorithm \cite{gelman2004}.
...
More specifically, we seek for such a value of $\vparam$ that maximizes \eref{posterior}. In this paper, this maximization is done via the Quasi-Newton algorithm with BFGS updates of the Hessian matrix \cite{press2007}.
