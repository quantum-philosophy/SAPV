...
Let $\specification$ be a description of the production setup including: (a) the layout of the wafer, (b) the floorplan of a single die on the wafer, and (c) the thermal parameters of the materials that the dies are made of.
...
The inputs to our framework should be composed of (a) a description of the production setup $\specification$, (b) a workload given as a detailed dynamic power profile $\profilePdyn$, and (c) prior knowledge on $\u$.
...
To approach the established goal, we propose the use of indirect measurements. Specifically, we aim at measuring temperature, which we shall analyze using Bayesian inference. Thus, as the first step, the user is supposed to harvest, as described in \sref{motivation}, a set of temperature profiles, corresponding to $\profilePdyn$, at several locations on the wafer. Denote by
\[
