...
Then, following \aref{kl-expansion}, the dimensionality of the \qoi\ is reduced even further by factorizing the covariance matrix of $\vu$, computed via \eref{covariance-function}, resulting in the following decomposition:
\begin{equation} \elabel{kl-approximation}
  \vu = \mu_\u \vI + \sigma_\u \mKL \vz
\end{equation}
where the final dimensionality is denoted by $\nvars$, $\vI = (e_i = 1) \in \real^{\ndies \nprocs}$, $\mKL \in \real^{\ndies \nprocs \times \nvars}$, and $\vz = (\z_i) \in \real^\nvars$ obey the standard Gaussian distribution.
Typically, $\nvars \ll \ndies \nprocs$.
Consequently, the model of $\u$ is now ready for practical computations.
In what follows, the parameters of \eref{u-prior} is defined by $\vparam_\u = \{ \vz, \mu_\u, \sigma^2_\u \}$ (see \fref{inference}).

\subsubsection{Likelihood function}
In the Bayesian context, the observed information is taken into account via a likelihood function (see \sref{bayesian-inference}).
In our case, the observed data are the temperature profiles in $\Data$, stacked into $\mvT_\meas$, which are modeled using the data model described in \sref{data-model}.
Now, since the measurement process is not perfect, we should take into consideration the measurement noise. To this end, for a given $\u$, the observed temperature $\mvT_\meas$ is assumed to deviate from the data model prediction $\mvT$ as follows:
...
\end{equation}
The probability density function of this distribution is the likelihood function $\f{\Data | \vparam}$ of our statistical model, which is the first of the two components needed for the posterior given in \eref{bayes}.
...
Taking the product of the densities in \eref{z-prior}--\eref{sigma2-noise-prior}, we obtain the prior $\f{\vparam}$, which is the second part of the posterior in \eref{bayes}.

\subsubsection{Posterior}
At this point, we have obtained the two pieces of the posterior shown in \eref{bayes}: the likelihood function, which is the density in \eref{likelihood}, and the prior, which is the product of the densities in \eref{z-prior}--\eref{sigma2-noise-prior}. Thus, the posterior is then
...
The problem, however, is that the direct sampling of the posterior is not tractable due to the data model involved in the likelihood function via the temperature vector $\mvT$ (see \eref{likelihood} and \sref{data-model}).
In order to circumvent this problem, we utilize the Metropolis-Hastings (MH) algorithm \cite{gelman2004} introduced in \sref{bayesian-inference}.
